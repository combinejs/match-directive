    ...
    constructor(expr) {
        ...
        switch (true) {
            case /^[1-9]\d*$/.test(expr):
                this._type     = Selector.TYPE_INDEX;
                this._params.a = parseInt(expr, 10);
                this._priority = 3;
                break;

            case /^-[1-9]\d*$/.test(expr):
                this._type     = Selector.TYPE_NEGATIVE_INDEX;
                this._params.a = parseInt(expr);
                this._priority = 2;
                break;

            case TYPE_PERIODIC_AN_B_REGEXP.test(expr):
                this._type     = Selector.TYPE_PERIODIC_AN_B;
            ...
            case /^\*$/.test(expr):
                this._type     = Selector.TYPE_EVERY;
                this._priority = 0;
                break;
            default:
...
Selector.TYPE_INDEX          = Symbol('TYPE_INDEX');
Selector.TYPE_NEGATIVE_INDEX = Symbol('TYPE_NEGATIVE_INDEX');
Selector.TYPE_PERIODIC_AN_B  = Symbol('TYPE_PERIODIC_AN_B');
Selector.TYPE_EVERY          = Symbol('TYPE_EVERY');
